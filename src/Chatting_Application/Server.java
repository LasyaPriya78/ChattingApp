...
public class Server implements ActionListener{
	...
	public Server() {
		...
		JLabel name = new JLabel("Sunny_bOy");
	...
	public void actionPerformed(ActionEvent e) {
      try { 
		...
		f.repaint();                 // for refreshing/reloading the frame
		f.invalidate();
		f.validate();
	...
	public static void main(String[] args) {
         new Server();
         try {
        	  ServerSocket sdk = new ServerSocket(6001);   //to make this class as server and server starts at port 6001
        	  while(true) {
        		  Socket s = sdk.accept();  // to accept msgs from client continuously
        		  DataInputStream din = new DataInputStream(s.getInputStream());
        		  dout = new DataOutputStream(s.getOutputStream());
        		  
        		  while(true) {
        			  String msg = din.readUTF();   //to read msgs from client continuously
        			  JPanel panel = FormatLabel(msg);    //this will format the msg like adding background padding etc like above
